...
class iliad_safey_metrics_saver(object):
    def __init__(self):
        ...
        # Subscribers
        rospy.Subscriber("robot3/human_perception/tracked_persons_in_base_link", TrackedPersons, self.tracked_persons_callback, queue_size=1)
        rospy.Subscriber("robot3/control/odom", Odometry, self.robot_odometry_callback)

        # take the start of the recording
        #rospy.Subscriber("active_robot_status",String, self.active_robot_status_callback,queue_size=1)
        rospy.Subscriber("/robot3/goal",PoseStamped, self.goal_callback,queue_size=1)
        
        # obtain the end of the mission -> trigger manually
        #rospy.Subscriber("mission_status", String, self.mission_status_callback,queue_size=1)
        

        rospy.Subscriber("robot3/control/controller/commands",ControllerCommand , self.controller_command_callback, queue_size=1)
        rospy.Subscriber("layer2_activated",Empty,self.layer2_activated_callback,queue_size=1)
        rospy.Subscriber("robot3/qsr/layer3_activated",Empty,self.layer3_activated_callback,queue_size=1)
        rospy.Subscriber("robot3/qsr/layer3_replan_triggered",Empty,self.layer3_replan_triggered_callback,queue_size=1)
    ...
    def finish_recording_callback(self, msg_data):  # this is used to trigger the end of the recording process
    ...
    def goal_callback(self, msg_data): # this is used to trigger the start of the recording process
