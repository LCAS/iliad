...
void connectCallback(message_filters::Subscriber<CameraInfo> &sub_cam,
    ...
    if(!pub_ground_plane.getNumSubscribers()) {
        ...
        sub_cam.unsubscribe();
        ROS_DEBUG("Ground Plane estimated: New subscribers. Unsubscribing depth.");
        sub_depth.unsubscribe();
    } else {
        ROS_DEBUG("Ground Plane estimated: New subscribers. Subscribing cam.");
        sub_cam.subscribe();
        ROS_DEBUG("Ground Plane estimated: New subscribers. Subscribing depth.");
        sub_depth.subscribe(it,sub_depth.getTopic().c_str(),1);
    }
    ROS_DEBUG("Ground Plane estimated: done");
}
