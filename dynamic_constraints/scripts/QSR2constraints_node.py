...
class DynamicConstraintsNode():
    ...
    def getDistToHuman(self, human_pose_in):
        ...
        try:
            human_pose_base = self.tfBuffer.transform(
                hpose, self.base_frame_id, self.tf_timeout)
    ...
    def updateVisuals(self):
        if (self.closest_human_pose) and (self.closest_human_twist):
            ...
            line.header.frame_id = self.closest_human_pose.header.frame_id
            ...
            #robotP = Point(self.robot_x, self.robot_y, self.closest_human_pose.pose.pose.position.z)
            ...
            text.header.frame_id = self.closest_human_pose.header.frame_id
            text.header.stamp = rospy.Time.now()
            text.ns = "descriptor"
            text.action = Marker.ADD
            text.pose.orientation.w = 1.0

            text.pose.position.x = xh0/2.0
            text.pose.position.y = yh0/2.0
            ...
            (isValid, state) = self.isValidState(xh0, yh0, xh1, yh1, xr0, yr0, xr1, yr1)
    ...
    def isValidState(self, xh0, yh0, xh1, yh1, xr0, yr0, xr1, yr1):
        ...
        if len(t) != 1:
            rospy.logerr("Node [" + rospy.get_name() +
                         "@isValidState] : response timestamp message lenght is not 1.")
            return (ans, state)
        t = t[0]
        ...
        if len(v) != 1:
            rospy.logerr("Node [" + rospy.get_name() +
                         "@isValidState] : response values message lenght is not 1.")
            return (ans, state)
        v = v[0]
        state = v.qsr.values()
        if len(state) != 1:
            rospy.logerr("Node [" + rospy.get_name() +
                         "@isValidState] : response state message lenght is not 1.")
            return (ans, state)
        state = state[0]
