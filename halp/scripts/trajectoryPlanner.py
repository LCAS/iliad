"""
This library should be a reimplementation of the matlab version traj_gen in
orunav_mpc package.

Use example:
States are (x,y,theta,phi), but phi is not considered in the analysis.
init_state = np.array( [ [0], [0], [0], [0] ]  )
end_state  = np.array( [ [0.02], [0.01], [0], [0] ]  )

L is length of the car
L = 0.68

K is the speed profile. Two positive numbers
k =  np.array( [ [4],[4] ] )

Time increment between samples
dt = 0.06

Time instants where state is obtained.
tspan = np.arange(0,tf+dt,dt)

(state,control) = plan_car_trajectory(L,init_state,end_state,k ,tspan,dt)
...
def poly3(qi, qf, vi, vf, t):
    """
     Using 3rd order polynomial generate position, velocity and acceleration
     profiles that satisfy the initial and final position and velocity constraints.
